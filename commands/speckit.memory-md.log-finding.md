...
If the finding is approved as durable memory after tracking, refresh the local cache with `npx speckit-memory refresh-memory` when the optimizer is enabled and available.
